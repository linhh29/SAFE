...
class Stem(nn.Module):
    ...
    """
    def __init__(self, img_size=224, in_dim=36, out_dim=768, act='relu'):
        super().__init__()
        self.convs = nn.Sequential(
            nn.Conv2d(in_dim, out_dim//4, 3, stride=2, padding=1),
            nn.BatchNorm2d(out_dim//4),
            act_layer(act),
            nn.Conv2d(out_dim//4, out_dim, 3, stride=2, padding=1),
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        # revise here if the shape of input image changes
        self.pos_embed = nn.Parameter(torch.zeros(1, channels, 8, 8))
    ...
    def forward(self, inputs):
        ...
        feature = x.clone().squeeze()
        # print(feature.shape)
        return self.prediction(x).squeeze(-1).squeeze(-1), feature


@register_model
def vig_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=2, drop_path_rate=0.0, drop_rate=0.0, num_knn=8, **kwargs):
...
if __name__ == '__main__':
    ...
    # print(model)
    # x = torch.ones((100, 36, 4, 4))
    # print(model(x).shape)
    summary(model.cuda(), (36, 32, 32))