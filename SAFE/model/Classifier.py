...
class CosNorm_Classifier(nn.Module):
    def __init__(self, in_dims, out_dims, scale=16, margin=0.5, init_std=0.001):
        ...
        self.scale = scale
        self.margin = margin
        self.weight = Parameter(torch.empty(out_dims, in_dims).cuda())
...
def create_cosNorm_model(in_dims=512, out_dims=1000):
...
def create_lws_model(feat_dim, num_classes=1000, stage1_weights=False, dataset=None, log_dir=None, test=False, *args):
